...
RSpec.describe Group, type: :model do
  context 'validations' do
    ...
    it 'is not valid with a duplicate name for the same user' do
      user = User.create(name: 'Test User', email: 'user@example.com', password: 'password')
      Group.create(name: 'Test Group', user:)
      group = Group.new(name: 'Test Group', user:)
      expect(group).not_to be_valid
    end
  end
  ...
  context 'custom validations' do
    it 'is not valid with an invalid icon format' do
      user = User.create(name: 'Test User', email: 'user@example.com', password: 'password')
      group = Group.new(name: 'Test Group', user:)
      group.icon.attach(io: File.open(Rails.root.join('spec', 'fixtures', 'invalid_icon.txt')),
                        filename: 'invalid_icon.txt')
      expect(group).not_to be_valid
    end
  end
  ...
  context 'scopes' do
    it 'returns groups in descending order for a user' do
      user = User.create(name: 'Test User', email: 'user@example.com', password: 'password')
      groups = [
        Group.create(name: 'Group 1', user:, created_at: 3.days.ago),
        Group.create(name: 'Group 2', user:, created_at: 2.days.ago),
        Group.create(name: 'Group 3', user:, created_at: 1.day.ago)
      ]
